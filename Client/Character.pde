...
static class Character{
  ...
  public int addDrawEvent(DrawEvent newEvent){
    ...
    newEvent.z = currentDrawEvents.indexOf(newEvent)*10;
    return moveEventQueue.size();  
  } // addDrawEvent
  ...
  protected void executeMoveEvent(){
    ...
    switch (currentMoveEvent.type){
      ...
      case 1:    // 移動イベント(速度指定)
      {
        ...
        if (currentMoveEvent.startTick + currentMoveEvent.duration < app.millis()){
      ...
      case 2:    // 移動イベント(目的地へ移動)
      {
        ...
        if ( distance < 0.1f 
          || currentMoveEvent.startTick + currentMoveEvent.duration < app.millis()){
          currentMoveEvent.flagFinished = true;
        }
        println("x:" + x + " y:" + y + " dist:" + distance);
        //println(currentEventIndex + " MoveTo: (" + moveEvent.x + ", " + moveEvent.y + ") " + (moveEvent.startTick + moveEvent.duration - app.millis()) + "mSec left");
        break;
      }        
      case 3:    // 停止イベント
        // nop
        //println("Waiting: " + (currentMoveEvent.startTick + currentMoveEvent.duration - app.millis()) + "mSec left");
        if (currentMoveEvent.startTick + currentMoveEvent.duration < app.millis()){
          currentMoveEvent.flagFinished = true;
        }
        break;        
      ...
      case 7:  // 攻撃イベント（ショット）
      {
        ...
        bulletManager.shootBullet(0,x+70*cos(radians(angle)),y+70*sin(radians(angle)),10*cos(radians(angle)),10*sin(radians(angle)));
        currentMoveEvent.flagFinished = true;
        break;
      }
      case 8:  //攻撃イベント　（レーザー）
        float angle = currentMoveEvent.useLocalAxis ? (currentMoveEvent.shootAngle + rotation) : currentMoveEvent.shootAngle;
        bulletManager.shootBullet(0,x+70*cos(radians(angle)),y+70*sin(radians(angle)),10*cos(radians(angle)),10*sin(radians(angle)),"laser");
        if (currentMoveEvent.startTick + currentMoveEvent.duration < app.millis()){
          currentMoveEvent.flagFinished = true;
        }
        break; 
  ...
  protected void executeDrawEvent2(){
    ...
    /* フレームの遷移 */
    if (frameTick + frameInterval < app.millis()){
    ...
    for(int i = 0; i < currentDrawEvents.size(); i++){
      ...
      switch (event.type){
        ...
        case 1:  // 1: Ellipse
          app.ellipse(event.x, event.y, event.w, event.h);
          //println("Ellipse: " + event.r + " " + event.g + " " + event.b + " " + event.a);
          break;
          
        case 2:  // 2: Rectangle
          app.rect(event.x, event.y, event.w, event.h);
          //println("Rect: " + event.r + " " + event.g + " " + event.b + " " + event.a);
          break;
          
        case 3:  // 3: Color
          app.fill(event.r, event.g, event.b, event.a);
          //println("Color: " + event.r + " " + event.g + " " + event.b);
          break;
          
        case 4:  // 4: Triangle
          app.triangle(event.x,event.y,event.w,event.h,event.r,event.g);
          break;
          
        case 5:  // 5: Line
          app.line(event.x,event.y,event.w,event.h);          
          break;
          
        case 8 : // 3/15　星
          app.beginShape();
            for(int j = 0; j <= 5; j++){
              app.vertex(event.x+event.r*cos(radians(18+144*j+180+event.h)),event.y+event.r*sin(radians(18+144*j+180+event.h)));
            }
          app.endShape();
          break;
          
        case 9: // 3/15 多角形
          //app.rotate(radians(event.a+180));
          if(event.n%4 == 0){
            app.beginShape();
              for(int j = 0; j <= event.n; j++){
                app.vertex(event.x+event.r*cos(radians(90.0*(event.n-4)/event.n+720.0/event.n*j+180+event.h)),event.y+event.r*sin(radians(90.0*(event.n-4)/event.n+720.0/event.n*j+180+event.h)));
              }
            app.endShape();
            app.beginShape();
              for(int j = 0; j <= event.n; j++){
                app.vertex(event.x+event.r*cos(radians(360.0/event.n+90.0*(event.n-4)/event.n+720.0/event.n*j-180-event.h)),event.y-event.r*sin(radians(360.0/event.n+90.0*(event.n-4)/event.n+720.0/event.n*j-180-event.h)));
              }
            app.endShape();
          }
          else if(event.n%2 == 0){
            ...
            app.beginShape();
              for(int j = 0; j <= event.n; j++){
                app.vertex(event.x-event.r*cos(radians(360.0/event.n+90.0*(event.n-4)/event.n+720.0/event.n*j+180+event.h)),event.y-event.r*sin(radians(360.0/event.n+90.0*(event.n-4)/event.n+720.0/event.n*j+180+event.h)));//180
              }
            app.endShape();
          }
          else{
        ...
        case 11:
          app.beginShape();
            for(int j = 0; j <= event.n; j++){
              app.vertex(event.x+event.r*cos(radians(90.0*(event.n-4)/event.n+360.0/event.n*j+180+event.h)), event.y+event.r*sin(radians(90.0*(event.n-4)/event.n+360.0/event.n*j+180+event.h)));
            }
          app.endShape();
          break;
        ...
        case 12:
          ...
          app.arc(event.x,event.y,event.r,event.r,radians(startAngle),radians(endAngle),PIE);  
          break;
  ...
  public void move(){
    ...
    if(x<=-app.width/2)x=app.width/2-1;
    else if(x>=app.width/2)x=-app.width/2+1;
    else if(y<=-app.height/2)y=app.height/2-1;
    else if(y>=app.height/2)y=-app.height/2+1;  //画面内に収まるように
  ...
  protected boolean isInside(float x, float y){//あたり判定 3/20
    ...
    for(int i = 0; i < currentDrawEvents.size(); i++){
      ...
      // app.fill(0); app.ellipse(this.x + ex, this.y + ey, 200, 200); // あたり判定デバッグ用
