  ...
  protected void move(){
    ...
    for ( int i = 0; i < bulletManager.maxBullets; i++ ){
      if ( bulletManager.bulletArray[i] != null ){
        ...
        if (dist(x,y,b.x,b.y)<=diameter/2&&b.handID!=handID) {
          hp = 0;}
      }                  
    }
  } // move
...
if(shootType == "laser") {
    ...
    else if(shootType == "bomb")fill(255,0,0);
    else if(shootType == "free")fill(255,0,255);
    else if(shootType == "ship")fill(0,0,255);
    else fill(0);
  ...
  protected void shoot(float x, float y, float velocityX, float velocityY){
    ...
    startTime = millis();//processing内での時間の単位はms（ミリ秒）である。
  } // shoot
